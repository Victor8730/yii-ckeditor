...
class CKEditorWidget extends CInputWidget
{
    ...
    private $allowedLanguages = [
        'af', 'ar', 'bg', 'bn', 'bs', 'ca', 'cs', 'da', 'de', 'el', 'en', 'en-au', 'en-ca', 'en-uk', 'eo', 'es', 'et',
        'eu', 'fa', 'fi', 'fo', 'fr', 'fr-ca', 'gl', 'gu', 'he', 'hi', 'hr', 'hu', 'is', 'it', 'ja', 'km', 'ko', 'lt',
        'lv', 'mn', 'ms', 'nb', 'nl', 'no', 'pl', 'pt', 'pt-br', 'ro', 'ru', 'sk', 'sl', 'sr', 'sr-latn', 'sv', 'th',
        'tr', 'uk', 'vi', 'zh', 'zh-cn'
    ];

    private $options = [];
    private $allowedEditorTemplates = ['full', 'basic', 'advanced'];
    ...
    public function setLanguage($value)
    {
        ...
        $lng = (($p = strpos($value, '_')) !== false) ? str_replace('_', '-', $value) : $value;

        if (in_array($lng, $this->allowedLanguages)) {
            $this->language = $lng;
        } else {
            $this->language = 'en';
        }
    ...
    public function setEditorTemplate($value)
    {
        if (!in_array($value, $this->allowedEditorTemplates))
            throw new CException(Yii::t(__CLASS__, 'editorTemplate must be one of {temp}', ['{temp}' => implode(',', $this->validEditorTemplates)]));
